...
const FirestoreService = {
      ...
      const unsubscribe = onSnapshot(
        ...
        (querySnapshot) => {
          ...
          querySnapshot.forEach((doc) => {
            const data = doc.data();
            if (
              data.evento.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  async searchCollectionEventosStatus(
    collectionName,
    idDiretor,
    status,
    searchTerm
  ...
  async eventoAberto(path, idDiretor) {
    ...
    ); // Replace with your query conditions
      ...
      const unsubscribe = onSnapshot(
        ...
        (querySnapshot) => {
          ...
          querySnapshot.forEach((doc) => {
            const data = doc.data();
            if (
              data.descricao.toLowerCase().includes(searchTerm.toLowerCase())
            ) {
              documents.push(data);
            }
          });
